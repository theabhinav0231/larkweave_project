...
try {
    // The variable firebase_config is injected by Flask as a JSON string
    // It needs to be parsed
    firebaseConfig = JSON.parse(window.firebaseConfigFromServer); // See HTML change below
} catch (e) {
...
if (firebaseConfig && !firebase.apps.length) {
    ...
    console.log("Firebase initialized from server config.");
     ...
     console.error("Firebase config not provided by server.");
     // Prevent further Firebase interactions if config is missing
} else {
